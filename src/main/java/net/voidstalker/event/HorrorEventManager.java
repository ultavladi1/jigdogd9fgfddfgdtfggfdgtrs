...
public class HorrorEventManager {
    ...
    private static void onWorldTick(ServerWorld world) {
        ...
        if (!config.horrorEventsEnabled) return;
        if (world.getTime() % 20 != 0) return; // once per second
    ...
    private static void triggerRandomEvent(ServerWorld world, ServerPlayerEntity player, VoidStalkerConfig config, Random random) {
        ...
        if ((acc += config.darknessEventChance) > roll) {
            ...
            stalkerEvent(world, player, config, random);
        } else if ((acc += config.distantSoundEventChance) > roll) {
            distantSoundEvent(world, player, random);
        } else {
            staticNoiseEvent(world, player, random);
        }
    }
